...
- `modules/nixos/healthchecks-runitor.nix` — oneshot+timer per check, each wraps one command with `runitor`; exit code drives /success vs /fail. Slugs `${host}-{system,disk,smart,restic}`; system check fires every 30min and doubles as liveness. Restic pings via `OnSuccess`/`OnFailure` (can't wrap); the `hc-fail@` ping unit pipes `journalctl -u $MONITOR_UNIT` as the body. Ping key TPM-sealed at `/etc/healthchecks/ping-key.cred` (prodesk: `/persist/...` via impermanence). Checks in `tofu/healthchecks.tf`.
